...
class AlexNet(nn.Module):
    ...
    def __init__(self, in_channel=1, out_channel=10):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(in_channel, 64, kernel_size=5, stride=4, padding=2),# Input size 32*32*1
...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = os.path.join("/home/mani/Desktop/IJCNN 2022/Bear Fault Detection", 'Fault_AlexNet_1D.pt') # Define Path to save the model 
...
plt.tight_layout()
plt.savefig('Adv_Fault_AlexNet_CM_CW.png')
