...
class BiLSTM(nn.Module):
    def __init__(self, in_channel=1, out_channel=10):
        ...
        self.hidden2label2 = nn.Linear(self.hidden_dim * 4, Num_Class)
    ...
    def forward(self, x):
        ...
        bilstm_out=torch.flatten(bilstm_out, start_dim=1)
...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = os.path.join("/home/mani/Desktop/IJCNN 2022/Bear Fault Detection", 'Fault_BILSTM_2D.pt') # Define Path to save the model 
...
plt.tight_layout()
plt.savefig('Fault_BILSTM_2D_CM.png')
#plt.show()
import torchattacks
...
plt.tight_layout()
plt.savefig('Adv_Fault_BILSTM_CM_FGSM.png')