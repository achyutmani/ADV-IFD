...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    epoch_acc = 0
    count=0
    model.eval() # call model object for evaluation s
...
def evaluate1(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    for (x,y) in iterator:
        ...
        TS=adv_images.detach().cpu().numpy()
        #Dataset_Pathname="/home/mani/Desktop/IJCNN 2022/Bear Fault Detection/FGSM Attack Fault Dataset/Train FGSM Attack Files"
        TS_Name="Test_TS_CW"+str(count)
        #TS_Name=os.path.join(Dataset_Pathname+TS1_Name)
        np.save(TS_Name,TS)
